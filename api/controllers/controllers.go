...
var DebateMapCollection *mongo.Collection = database.ProductData(database.Client, "debatemaps")
...
func AddDebateMap(c echo.Context) error {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		var debatemaps models.DebateMap
		defer cancel()
		if err := c.Bind(&debatemaps); err != nil {
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
		}
		debatemaps.ID = primitive.NewObjectID()
		_, anyerr := DebateMapCollection.InsertOne(ctx, debatemaps)
		if anyerr != nil {
			return c.JSON(http.StatusInternalServerError, "Not Created")
		}
		defer cancel()
		return c.JSON(http.StatusOK, "マップを投稿しました")
	}

func debatemaps_list(c echo.Context) error {
...
func SearchDebateMapByQuery(c echo.Context) error{
		...
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		defer cancel()
		searchquerydb, err := DebateMapCollection.Find(ctx, bson.M{"product_name": bson.M{"$regex": queryParam, "$options": "i"}})
		if err != nil {
